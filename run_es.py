...
# ES
# ES type
parser.add_argument('--es', type=str, default='es', help='ES type', choices=['open', 'canonical'])
...
from qdax.core.es_parts.es_utils import ES
from qdax.core.es_parts.open_es import OpenESEmitter, OpenESConfig
from qdax.core.es_parts.canonical_es import CanonicalESConfig, CanonicalESEmitter
...
if args.es in ["open", "openai"]:
    ...
    es_emitter = OpenESEmitter(
        ...
        total_generations=args.num_iterations,
...
num_loops = int(args.num_iterations / log_period)
...
csv_logger = CSVLogger(
    log_file,
    header=["loop", "generation", "qd_score", "max_fitness", "coverage", "time", "frames"],
)
...
if args.plot:
    ...
    env_steps = jnp.arange(args.num_iterations) * args.episode_length
