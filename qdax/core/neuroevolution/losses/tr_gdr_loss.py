""" Implements a function to create critic and actor losses for the TD3 algorithm with Trust Region Gentic Distance Regularization"""
...
def tr_gdr_loss_fn(
    ...
    def _policy_loss_fn(
        ...
        g = jnp.sqrt(elastic_loss_sum) - gdr_state.tr_gdr_d

        policy_loss = policy_loss + gdr_state.tr_gdr_lambda * g
        return policy_loss
    ...
    def _lambda_loss_fn(
        ...
        """Loss function for the lambda parameter"""
        # L2 regularization on the distance between the actor and the center of an ES
        elastic_loss = jax.tree_util.tree_map(
            lambda x, y: ((x - y) ** 2), 
            policy_params,
    ...
    return _policy_loss_fn, _critic_loss_fn, _lambda_loss_fn