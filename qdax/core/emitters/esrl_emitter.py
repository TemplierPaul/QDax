...
class ESRLEmitter(Emitter):
    ...
    def multi_eval(
        ...
        genotypes = jax.tree_util.tree_map(
            lambda x: jnp.repeat(jnp.expand_dims(x, axis=0), 48, axis=0), genome
        )
        eval_fn = self.es_emitter._eval_fn
        fitnesses, descriptors, extra_scores, random_key = eval_fn(
    ...
    def surrogate_evaluate(
        ...
        transitions, random_key = replay_buffer.sample(
            subkey, sample_size=self._config.rl_config.surrogate_batch
        )
        ...
        transitions = self.rl_emitter.get_dummy_batch(
            self._config.es_config.sample_number, self._config.es_config.episode_length
        )
