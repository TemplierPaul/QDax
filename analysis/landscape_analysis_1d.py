...
def multi_evals(genome, n_evals=10):
    ...
    nets = jax.tree_map(lambda x: jnp.repeat(x[None, ...], n_evals, axis=0), net)
...
def batch_eval(genomes, n_evals=20, buffer=None):
    ...
    print(
        ...
        "batches of",
        batch_eval,
        "evaluations each. Total:",
        num_batches * batch_eval,
    )
    ...
    for i in tqdm(range(num_batches)):
        ...
        small_trans = jax.tree_map(lambda x: x[:, 0, ...], transitions)
...
if __name__ == "__main__":
    ...
    for gen in gens:
        ...
        fit, replay_buffer = batch_eval(
            genomes, n_evals=n_evals, buffer=base_replay_buffer
        )
        fitnesses[gen] = fit
